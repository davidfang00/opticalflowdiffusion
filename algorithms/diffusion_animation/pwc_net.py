...
class PWCNet(nn.Module):
    ...
    def corr(self, input1, input2):
        corr_fn = SpatialCorrelationSampler(kernel_size=1, patch_size=9, stride=1, padding=0, dilation=1, dilation_patch=1)
        out = corr_fn(input1, input2).flatten(1, 2)
        return out
    ...
    def forward(self, im_tar, im_refs):
        ...
        feat5b_warped = warp(feat5b, 0.625*flow6_fwd_up)
        feat5c_warped = warp(feat5c, -0.625*flow6_bwd_up)
        ...
        feat4b_warped = warp(feat4b, 1.25*flow5_fwd_up)
        feat4c_warped = warp(feat4c, -1.25*flow5_bwd_up)
        ...
        feat3b_warped = warp(feat3b, 2.5*flow4_fwd_up)
        feat3c_warped = warp(feat3c, -2.5*flow4_bwd_up)
        ...
        feat2b_warped = warp(feat2b, 5.0*flow3_fwd_up)
        feat2c_warped = warp(feat2c, -5.0*flow3_bwd_up)
        ...
        past_ds1 = im_norm[1]
        past_ds2 = F.interpolate(past_ds1, scale_factor=0.5, mode = 'bilinear')
        past_ds3 = F.interpolate(past_ds2, scale_factor=0.5, mode = 'bilinear')
        past_ds4 = F.interpolate(past_ds3, scale_factor=0.5, mode = 'bilinear')
        past_ds5 = F.interpolate(past_ds4, scale_factor=0.5, mode = 'bilinear')

        fut_ds1 = im_norm[2]
        fut_ds2 = F.interpolate(fut_ds1, scale_factor=0.5, mode = 'bilinear')
        fut_ds3 = F.interpolate(fut_ds2, scale_factor=0.5, mode = 'bilinear')
        fut_ds4 = F.interpolate(fut_ds3, scale_factor=0.5, mode = 'bilinear')
        fut_ds5 = F.interpolate(fut_ds4, scale_factor=0.5, mode = 'bilinear')

        tar_ds1 = im_norm[0]
        tar_ds2 = F.interpolate(tar_ds1, scale_factor=0.5, mode = 'bilinear')
        tar_ds3 = F.interpolate(tar_ds2, scale_factor=0.5, mode = 'bilinear')
        tar_ds4 = F.interpolate(tar_ds3, scale_factor=0.5, mode = 'bilinear')
        tar_ds5 = F.interpolate(tar_ds4, scale_factor=0.5, mode = 'bilinear')

        past_ds = [past_ds1, past_ds2, past_ds3, past_ds4, past_ds5]
        fut_ds = [fut_ds1, fut_ds2, fut_ds3, fut_ds4, fut_ds5]
        tar_ds = [tar_ds1, tar_ds2, tar_ds3, tar_ds4, tar_ds5]
        ...
        for i in range(len(flow_fwd)):
            warp_fwd_bwd = [warp(fut_ds[i], flow_fwd[i]), warp(past_ds[i], flow_bwd[i])]
            warped_imgs.append(warp_fwd_bwd)

        return flow_fwd, flow_bwd, occ, warped_imgs, tar_ds
