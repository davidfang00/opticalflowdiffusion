...
def softsplat(
    ...
    tenIn: input tensor
    tenFlow: flow tensor
    tenMetric: metric tensor
    strMode: mode
    
    return: A warped tensor along with the warped metric tensor in the last dim
    '''
    ...
    if strMode.split("-")[0] in ["avg", "linear", "soft"]:
        ...
        tenOut = torch.cat((tenOut[:, :-1, :, :] / tenNormalize, tenOut[:, -1, None, :, :]), dim=1)
    # end
