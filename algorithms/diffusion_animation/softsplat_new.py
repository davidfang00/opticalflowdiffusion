...
def softsplat(
        tenIn: torch.Tensor, tenFlow: torch.Tensor, tenMetric: torch.Tensor, strMode: str, scale: int, offset: tuple 
):
    assert strMode.split("-")[0] in ["sum", "avg", "linear", "soft", "linear_unn"]
    ...
    tenOut = softsplat_func.apply(tenIn, tenFlow, scale, offset[0], offset[1])
    ...
    if strMode.split("-")[0] in ["avg", "linear", "soft"]:
        ...
        tenRet = torch.cat((tenOut[:, :-1, :, :] / tenNormalize, tenOut[:, -1, None, :, :]), dim=1)
    else:
        tenRet = tenOut
    # end
...
class softsplat_func(torch.autograd.Function):
    ...
    def forward(self, tenIn, tenFlow, scale, offset_x, offset_y):
        tenOut = tenIn.new_zeros(
            [tenIn.shape[0], tenIn.shape[1], tenIn.shape[2] // scale, tenIn.shape[3] // scale]
        )
    ...
    def backward(self, tenOutgrad):
        ...
        return tenIngrad, tenFlowgrad, None, None, None
